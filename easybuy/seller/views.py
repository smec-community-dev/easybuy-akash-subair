...
def seller_dashboard(request):
    ...
    products = Product.objects.filter(seller=seller).prefetch_related('variants', 'variants__images')
    return render(request, 'seller/dashboard.html', {'seller': seller, 'products': products})



def seller_product_list(request):
    """Seller Inventory View - Shows products with variants"""
    seller = request.user.seller_profile
    ...
    return render(request, 'seller/inventory.html', {'seller': seller, 'products': products})
...
def seller_inventory(request):
    ...
    try:
        ...
        seller = None
    if seller:
        # Get all products for this specific seller with related data
        products = Product.objects.filter(
            seller=seller,
            is_active=True
        ).prefetch_related(
        ...
        for product in products:
            for variant in product.variants.all():
                ...
                total_inventory_value += float(variant.selling_price) * variant.stock_quantity
